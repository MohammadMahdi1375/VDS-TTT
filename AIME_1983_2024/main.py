...
def best_of_n_oracle(dataset, config: Config, llm: LLM, N=Config.n, save_results=False):
    ...
    for i, data in progress_bar:
        ...
        completion_tokens = [[] for _ in range(len(x["prQuestionoblem"]))]
...
def PRM_pass_N(x, config: Config, llm: LLM, prm: PRM, N, save_results=False):
    ...
    print("########################################################################################")
    print(f"Accuracy of LLaMA-3.2-3B-PRM@{N}: {n_true_ans/n_samples:.4f}.")
    print(f"Elapsed time for evaluating the base model is: {end-start:.2f} secs.")
    print("########################################################################################")

    ################################################ _Deepseek_R1_Qwen_1.5B 
    if save_results:
        with open("./TTT_data/Best_of_" + str(N) + "_Deepseek_R1_Qwen_1.5B_KK3.json", mode="w") as file:
            json.dump(table, file, indent=4)
#############################################################################################################
########################################## Evaluation: ["Pass@1"] ###########################################
#############################################################################################################
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset_repo_name",      default="gneubig/aime-1983-2024",                           help="Reward model path")
