...
parser.add_argument("--n_repetitive_sampleing", default=16,         help="Number of repetitive sampling")
...
parser.add_argument("--WARM_UP_RATIO",          default=0.05,       help="Ratio of total training steps used for a linear warmup")
parser.add_argument("--WEIGHT_DECAY",           default=0.01,       help="Whether apply the Lora or not")
parser.add_argument("--n_train_samples",        default=500,        help="Number of testing data")
parser.add_argument("--Score_Filter",           default=0.9,        help="score below which data is filtered")
...
dataset = pd.read_json("./TTT_data/Best_of_" + str(args.n_repetitive_sampleing) + "Llama_1B.json", orient='records')
dataset = dataset[dataset['score'] > args.Score_Filter]
args.n_train_samples = len(dataset['score'] > args.Score_Filter)

hf_dataset = Dataset.from_pandas(dataset)
hf_dataset.save_to_disk("./TTT_data/HF_Best_of_" + str(args.n_repetitive_sampleing) + "Llama_1B")
