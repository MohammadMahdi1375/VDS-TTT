    ...
    for i, data in progress_bar:
        if (i >= index):
            ...
            if match:
                model_answer = match.group(1)
                try:
                    if np.float64(true_answer) == np.float64(model_answer):
                        n_true_ans += 1
                except ValueError:
                    pass
            ...
            if save_results:
                with open("./TTT_data/Best_of_" + str(N) + "Llama_1B.json", mode="w") as file:
                    json.dump(table, file, indent=4)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset_repo_name",      default="AI-MO/aimo-validation-amc",                    help="Reward model path")
    ...
    if (args.evaluation_choice == 1):
        ...
        PRM_pass_N(dataset, index=0, config=config, llm=llm, prm=prm, N=config.n, save_results=args.save_to_json)
